...
export const RecipeCard = ({ recipe }: RecipeProps) => {
  const { ingredientsCount, totalTime, name, image, rating, url, ratingCount } = recipe;
  const navigate = useNavigate();
  return (
    <div
      className="flex flex-col w-70 min-h-80 border-2 rounded-lg overflow-hidden shadow-md transition-transform duration-300 hover:scale-105 hover:shadow-lg cursor-pointer"
